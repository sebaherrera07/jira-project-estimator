...
RSpec.describe EpicProgressPresenter do
  describe '#total_points' do
    subject { described_class.new(issues: issues).total_points }
    ...
    context 'when epic has estimated issues' do
      ...
      it 'returns the sum of points' do
        expect(subject).to eq(estimated_issues.sum(&:points))
      end
    end
  end

  describe '#completed_points' do
    subject { described_class.new(issues: issues).completed_points }
    ...
    context 'when epic has completed and estimated issues' do
      ...
      it 'returns the sum of points' do
        expect(subject).to eq(completed_estimated_issues.sum(&:points))
      end
    end
  end

  describe '#remaining_points' do
    subject { described_class.new(issues: issues).remaining_points }
    ...
    context 'when epic has issues' do
      ...
      it 'returns the sum of remaining points' do
        expect(subject).to eq(
          to_do_estimated_issues.sum(&:points) + in_progress_estimated_issues.sum(&:points)
        )
      end
    end
  end
  ...
  describe '#earned_value' do
    ...
    context 'when total_points is 0' do
      it 'returns 0%' do
        allow_any_instance_of(described_class).to receive(:total_points).and_return(0)
        expect(subject).to eq('0%')
      end
    end

    context 'when total_points is not 0' do
      context 'when completed_points is 0' do
        it 'returns 0%' do
          allow_any_instance_of(described_class).to receive(:total_points).and_return(35)
          allow_any_instance_of(described_class).to receive(:completed_points).and_return(0)
          expect(subject).to eq('0%')
        end
      end

      context 'when completed_points is not 0' do
        it 'returns the progress percentage' do
          allow_any_instance_of(described_class).to receive(:total_points).and_return(35)
          allow_any_instance_of(described_class).to receive(:completed_points).and_return(11)
